...
§
自研项目本地工作区在 D:\ai coding\GitRepos\<name>（如 tweakbyjie、workbuddy2api），分析/改码前先查本地工作区，勿直接克隆 GitHub。workbuddy2api = 本地反代 8787（converter.py 内核 + Tauri 壳），改动须走 TDD + 逐点变异门槛（N 处落点 = N 条直接契约）。
§
