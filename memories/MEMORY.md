...
§
Hermes 配额监控与桌面美化内置化演进（2026-09-05）：token-stats 插件（FastAPI 路由 /api/plugins/token-stats/ 随桌面后端挂载）已集成左侧 Pulse 导航、/quota 看板、WorkBuddy 8787 探测与 /quota 斜杠指令；装配 zcode-theme 与 14 款暗黑主题。另打通对 ZCode 本地 SQLite 库（~/.zcode/cli/db/db.sqlite）实时毫秒级进度、CoT 思考与子代理状态监听，支持双 Agent 协同与异常接管。
§
