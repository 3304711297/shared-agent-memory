...
§
工作流与并发：动手时全面优先使用并发以最大化效率；多任务/多仓库/大批量检查修复场景优先调度并行子代理（Gemini 实测单批支持 10 并发，日常推荐 3~6 个一批并行推进，彻底解除旧 2 并发限制）。
§
铁律：不确定技术细节必严格优先联网检索（官方文档/知识库）实证事实，严禁凭空臆测；代码/文档推送到 GitHub 后必主动跟踪监控 Actions CI 全绿后方可结束对话，严禁未等 CI 提前收尾。
§
跨 Agent 协同自驱接手铁律（2026-09-05 确立）：当需要等待 ZCode 或外部长跑任务完成并接手时，严禁口头承诺后休眠等待用户提醒，也严禁前台死循环阻塞；必须使用 terminal(command="python C:/Users/<username>/AppData/Local/hermes/scripts/watch_zcode.py", background=True, notify=True) 在后台挂起轻量守护监听，退出时由系统事件通知唤醒，自动无缝派发多子代理接手审查与推进。
§
