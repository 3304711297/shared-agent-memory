...
- ZCode CLI path: `C:\Users\<username>\.zcode\cli\`
- Memory path: `C:\Users\<username>\.zcode\cli\memories\projects\default-135ef1b9f66d8a7e\memory\`
...
- Plugin installed at: `C:\Users\<username>\.zcode\cli\plugins\cache\claude-plugins-official\adobe-for-creativity\2.0.0\`
...
- **运行方式**：本地模型网关监听 `http://127.0.0.1:18080`，官方核心内嵌正版 OAuth 凭据，直接原生支持 `gemini-3.8-flash`（及 `-high` 思维链）、`gemini-3.7-flash`、`gemini-3.6-flash`、`gemini-3.1-pro-low`、`gemini-web-search` 等全系模型。
- **协议双通**：
  - Hermes Agent 走 OpenAI 兼容协议（`http://127.0.0.1:18080/v1/chat/completions`，提供商 `cpa-gui`）；
  - ZCode 走 Anthropic Messages 协议（`http://127.0.0.1:18080/v1/messages`，提供商 `zcode-antigravity-local`）。
