...
- [User Windows Environment](user-windows-environment.md) — Windows 环境：本地代理 127.0.0.1:3067、gh 账号 3304711297、PS 5.1/7 双版本规则、浏览器是 Edge Dev、EasyCLIProxyAPI 官方网关(18080端口，Gemini 3.8/3.7主控+ZCode双端接入已打通)、Google 配额监控已内置化（18088 微服务退役，见 hermes-quota-embedded）
- [Gateway Migration EasyCLIProxyAPI and Browser Protection](gateway-migration-easycliproxyapi-and-browser-protection.md) — 2026-09-04 架构升级复盘：EasyCLIProxyAPI (v7.2.149) 原生支持 Gemini 3.8/3.7，ZCode 双端配置与 NTFS 目录联接；浏览器扩展清空事故深度定性与沙箱隔离防灾；Google 配额 daily-cloudcode-pa 双端点隔离机制与桌面看板 Popover 现代 UI 重构规范全录
- [Hermes Quota Embedded](hermes-quota-embedded.md) — 配额监控内置化：token-stats 后端插件（/api/plugins/token-stats）替代 18088 微服务与计划任务，plugins.enabled 安全门、ctx.rest 前端门、三大登录自启任务已停用
