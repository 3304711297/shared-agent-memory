...
- **ZCode 专属分支**：`zcode`（本地目录 `C:\Users\<username>\.zcode\cli\memories\`）
  - 触发条件：修改或新增 `.zcode` 记忆文件后，执行 `cd "C:/Users/<username>/.zcode/cli/memories" && git add . && git commit -m "backup: 自动备份最新记忆" && git push`
- **Hermes 专属分支**：`hermes`（本地目录 `C:\Users\<username>\AppData\Local\hermes\`）
  - 触发条件：修改或新增 Hermes 记忆/技能后，执行 `cd "C:/Users/<username>/AppData/Local/hermes" && git add . && git commit -m "backup: 自动备份 hermes 记忆与配置" && git push`
