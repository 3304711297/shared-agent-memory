...
- **`main` 分支 = 双端共享记忆库唯一真源**：物理位于 `C:\Users\<username>\.zcode\cli\memories\`（本地检出分支），Hermes 经 NTFS 目录联接 `%LOCALAPPDATA%\hermes\memories\topics` 直读同一目录（见 [[hermes-shared-memory]]）。任何一方修改共享内容，当轮结束前执行：
  ```bash
