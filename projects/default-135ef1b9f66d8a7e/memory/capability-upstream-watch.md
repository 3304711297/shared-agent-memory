...
- **第二轮跟进与自动化收口闭环（Issue #3 & #4，2026-09-06 彻底收口归零）**：
  ...
  - **第八轮（Issue #14，2026-09-15 收口；2 项全清，均为「只回写基线」型）**：
    - **ponytail-skills 4.9.0→4.10.0（无文件需同步）**：核查结论=上游 `skills/` 子树逐字节未变。判据手法：克隆后比 `git rev-parse <旧tag>:skills <新tag>:skills`，两侧 tree hash 同为 `d15d0642`、6 个 SKILL.md 的 blob hash 逐一相同；上游改动全在 `hooks/`（Cursor 原生 hooks via hooks.json、Grok Build 适配器）与各插件 manifest/README。本地差异仅两类既定定制（中文强触发词 description 4 项、正文瘦身 2 项），均不得被上游覆盖。处理：只回写 `version`+`sha`。
    - **openviking 0.4.19→0.4.20（真实升级）**：pip 升级 Windows wheel，升级前先 `openviking_service.py stop` 并 wmic 复核无残留（只信 status 会踩坑）。**0.4.20 两处影响本机链路的行为变更**：① 记忆抽取默认输出协议 json→受限 Python DSL（`memory.extraction_output_format` 默认 `python`），本机 VLM 为 reasoning 占大头的 deepseek-v4.1-flash，故升级后**必须实发一条记忆并等 `ov task status` 到 completed** 验证抽取端到端可用（实测 memory_write 4 / memory_edit 2 正常）；② 抽取输出上限未显式配置时依次取 `vlm.max_tokens`、默认 32768。Compile 接口迁移（`/bot/v1/compile` 停用→`/api/v1/compile` + `/api/v1/tasks/{task_id}`）与 CLI 参数改名对本机无影响（无脚本依赖）。
    - **兼容性复核（本轮无脚本改动）**：`ov add-resource` 的 `--to`/`--parent`/`--wait` 形态未变，`sync_shared_memory_openviking.py` 依赖的命令升级后实测仍可用（实发一次 add-resource 成功，临时资源已清理）。
    - **可复用判据（本轮沉淀）**：看门报技能类组件「上游有新版」时，**版本号变化 ≠ 已装技能有变化**。先比子树 tree hash：相同则本次属「只回写基线」收口，严禁按版本号覆盖本地技能（会冲掉中文触发词与瘦身成果）。
