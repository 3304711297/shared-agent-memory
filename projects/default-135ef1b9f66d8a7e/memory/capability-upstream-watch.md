...
- **GUI「可更新」徽章与开关的两个坑（2026-09-05）**：①开关失效根因即上条 provider.npm 配置丢弃事故，配置修复后开关恢复（enabledPlugins 真值存 cli/config.json 未丢）；②徽章比对源是客户端本地市场快照 `plugins/marketplaces/claude-plugins-official/.claude-plugin/marketplace.json`（非 git 不会自更新）——手工换装后 pin ≠ 已装 sha 会误报可更新，定向补丁该文件的 sha 字段即可；③**徽章若在快照补丁+配置修复后仍显示：是 GUI 自己的安装记录（IndexedDB，记录装机时版本）未感知手工换装——直接点 GUI「更新」按钮即消失（内容与缓存逐字节一致，重装自市场 pin）**；⚠️ 点击更新后 plugin.json 会被重置为市场原版裸参数，必须重新补 `--ignore-default-chrome-arg=--disable-extensions` 保护参数。
