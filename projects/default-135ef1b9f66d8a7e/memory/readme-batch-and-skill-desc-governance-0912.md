...
## 一、11 仓库 README 批量治理
- 范围：GitRepos 全部 10 仓（含当时的 openviking-backup，该仓已于 2026-09-21 删除），修事实错误 + 补标准模块（结构树/FAQ/恢复步骤），全部推送且 CI 绿。
...
- 运维要点：改技能后须重命名 `.skills_prompt_snapshot.json` 快照缓存，新会话生效；机制详情见记忆卡 hermes-skill-description-truncation 一类条目 / 本节上文。
