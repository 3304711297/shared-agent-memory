...
- **会话管理、归档与清理终局铁律（2026-09-06 用户拍板）**：用户日常**明确只使用普通会话模式（default profile 一体化协作）**。当会话累计 ≥1M tokens 或用户下达「删除/归档当前会话」指令时，当前接待的 Agent 必须严格执行三步标准化收尾闭环 SOP（严禁仅口头附和后直接草草退出）：
  1. **沉淀高价值记忆与 ysk 用户决策**：
     - 深入提炼本会话产生的一切技术决策、故障根因与避坑教训，自动同步至双端共享记忆库（shared-agent-memory）；
     - **【ysk 写入铁律】严禁擅自直接向 youshouldknow 知识库写入内容**！必须整理出包含一句话理由的候选主题/选项清单供用户挑选拍板，仅在用户明确勾选/确认后方可写入并执行文档规范；
  2. **物理清理无用文件**：彻底扫描并物理删除本会话产生的全部临时/无用文件（例如测试脚本、探针脚本、抓取残留、临时日志等中间产物），严禁在磁盘遗留任何调试垃圾；
  3. **自动同步与收尾确认**：变动自动提交并推送到 GitHub（共享库推 main，专属内容推 hermes），本地优先全体验证全绿后直接完成收口并告知用户，CI 状态由后台异步跟踪，严禁卡死主会话。
...
- **任务可视化打点偏好（2026-09-08 用户确认）**：极度认可 Hermes 桌面端 `todo_list` 原生交互卡片的进度追踪体验。面对 3 步及以上的复杂工程、计划执行或批处理任务，严禁用纯文本 Markdown 列表敷衍，必须坚决调用 `todo_list` 原生工具建立任务状态机并随步骤推进实时更新状态（完成打勾、进行中标粗），让用户直观掌控当前阶段与整体进度。
...
- **2026-09-05 深夜 WorkBuddy glm-5.3-flash 复测**：切换聊天模型至 WorkBuddy（127.0.0.1:8787）后，API 直连 2/4/6/8/10/15 并发全部 100% 成功且零 429（10 并发 6.61s、15 并发 4.07s），delegate_task 3 并发子代理同样正常；**glm-5.3-flash 属思考型模型，max_tokens<100 时思考链耗尽配额导致 content=null**（实测脚本需给足 max_tokens≥300 或读 reasoning 字段）——旧「并发≈2」结论确认为当时 max_tokens 过小引发的假阳性，而非服务商限流。
