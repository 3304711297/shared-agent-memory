...
- **跨 Agent 协同自驱接手铁律（2026-09-05 确立）**：当需要等待 ZCode、外部长跑构建或长时间测试完成并自动接手时，**严禁口头承诺后休眠等待用户提醒**，也严禁在前台死循环阻塞会话；必须使用 `terminal(command="python C:/Users/<username>/AppData/Local/hermes/scripts/watch_zcode.py", background=True, notify=True)` 在后台挂起轻量守护监听脚本，退出时由系统通知唤醒，自动无缝派发多子代理接手审查推进。
