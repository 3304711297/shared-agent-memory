...
2026-09-05 用户拍板：**多仓库/多任务场景优先用并行子代理省时间**（并发受限时改串行排队），大批量检查、修复类工作均可拆组并行。 **实测并发上限≈2**：一次 4 并发被拒 2 个、3 并发被拒 1 个、2 并发稳定通过——批量派发按 2 个一批排队的节奏；gh 已于 09-05 授权 delete_repo scope（可直接 gh repo delete）。
