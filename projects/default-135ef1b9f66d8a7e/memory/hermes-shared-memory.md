...
- **唯一物理真源**：`C:\Users\<username>\.zcode\cli\memories\projects\default-135ef1b9f66d8a7e\memory\`（git `main` 分支检出）。
...
- **【铁律】自动推送**：任何 Agent（ZCode 或 Hermes）修改共享库后，当轮结束前必须在 `C:/Users/<username>/.zcode/cli/memories` 仓库提交并推送 `main` 分支（旧规则「hermes 只写不推、由 ZCode 代推」已作废——hermes 现在直接自行推 main）。
