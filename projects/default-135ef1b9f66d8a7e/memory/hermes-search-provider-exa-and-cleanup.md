...
## 3. 冗余社区搜索技能清理
- **溯源确认**：经对比官方内置清单 `.bundled_manifest` 与 `skills/.usage.json`，`duckduckgo-search`（作者 gamedevCloudy）与 `searxng-search` 均为 2026-09-03 后续安装的社区技能。
- **清理下线**：
  - `duckduckgo-search`：环境未装 `ddgs` 依赖，且 Hermes 原生已内置 DDG 驱动。
  - `searxng-search`：平台被限定为 `[linux, macos]` 且无本地/远程 `SEARXNG_URL`。
- **操作完成**：已通过 `hermes skills uninstall -y` 完成两项技能的彻底卸载，保持技能库纯净度。
