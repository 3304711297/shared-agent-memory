...
- **Hermes 桌面更新拦截「关闭其他进程以更新 Hermes」根治与常驻进程隔离铁律（2026-09-06 彻底闭环）**：
  ...
  - 根治措施：
    1. 进程环境彻底隔离：将开机自启脚本 `Startup\OpenVikingGateway.vbs` 和 `quota_service.cmd` 内的解释器全部改为独立虚拟环境 `C:\Users\<username>\.openviking\venv\Scripts\pythonw.exe`，终止旧进程并重启网关；
