...
- **用户适合 GUI 不适合 TUI/CLI（2026-09-02 实测反馈）**：官方 Electron 桌面端已构建并跑通——`hermes desktop` 首跑自动 npm 构建（需代理环境变量），产物稳定路径 `hermes-agent\apps\desktop\release\win-unpacked\Hermes.exe`；桌面快捷方式已改指此 exe（原 CLI 版启动方式作废）。`hermes dashboard` 是浏览器版配置管理页，`hermes serve` 是其无头后端。
