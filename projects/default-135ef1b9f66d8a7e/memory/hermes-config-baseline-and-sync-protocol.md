...
| **安装目录与方式** | `C:\Users\<username>\AppData\Local\hermes\hermes-agent` (Git source checkout) | 源码检出并可热更新 |
...
* **本地源文件路径**：`C:\Users\<username>\AppData\Local\hermes\config.yaml` (450行全量)
...
- **How to apply**: 当用户提及「我改了设置 / 看一下我改的 / 同步一下设置」时，立即执行 `read_file(C:/Users/<username>/AppData/Local/hermes/config.yaml)`，同步核对 `hermes --version`，对比变动点后覆写更新本文件与 `hermes-config.yaml` 并提交推送 `main`。
