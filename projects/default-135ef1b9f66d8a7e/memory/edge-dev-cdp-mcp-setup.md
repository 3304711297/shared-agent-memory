...
- **ZCode 配置**：`~/.zcode/cli/config.json` → `mcp.servers["chrome-devtools"]` = `npx -y chrome-devtools-mcp@1.7.0 --autoConnect --user-data-dir=%LOCALAPPDATA%\Microsoft\Edge Dev\User Data`。
...
**青柠起始页补装与根因再确认（2026-08-23 上午）**：用户指出扩展未装齐并再次强调 **Tampermonkey 完全不需要、勿装**。审计发现漏了**青柠起始页 / Lime Start Page（pcpnigdkpcgemocnjhebmajldpjlbeom，新标签页起始页，即 limestart.cn）**——首次盘点用了 `ls | head -8` 截断，字母序在 khkeo 之后的扩展（含它和当时在运行的脚本猫）被漏数；教训：**盘点扩展目录必须完整 ls，勿截断**。其商店安装同样报错，用 CRX 直链下载验证 manifest 也是 `"default_locale": "zh_CN"`——第二个 zh_CN 受害者，同时**推翻此前「本机 CRX 下载管线损坏」的猜测**（直链 26.6MB 下载完好）。已按同方补装：解压到 `%USERPROFILE%\Documents\EdgeExtensions\LimeStartPage\src`（**商店 CRX 解压后须删 `_metadata` 目录**再加载）、default_locale 改 en、用户手选文件夹加载（截至记录时待用户确认结果）。起始页个人配置靠 limestart.cn 账号云同步恢复。**CRX 直链（可绕商店 UI 下载检查）**：`https://edge.microsoft.com/extensionwebstorebase/v1/crx?response=redirect&prod=chromiumcrx&prodchannel=Stable&x=id%3D<扩展ID>%26installsource%3Dondemand%26uc`。
