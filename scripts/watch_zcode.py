...
Polls C:/Users/<username>/.zcode/cli/db/db.sqlite every 3 seconds.
...
DB_PATH = Path(r"C:\Users\<username>\.zcode\cli\db\db.sqlite")
...
def check_session_done(conn: sqlite3.Connection, session_id: str) -> bool:
    cur = conn.cursor()
    # Check if there are active subagents under this session
    ...
    if subagents:
        for sa in subagents:
            ...
            # If subagent is still executing tools or in step-start, it's not done
            ptype = pd.get("type")
    ...
    # Check main session latest part
...
def main() -> None:
    ...
    while time.time() - start_time < args.timeout:
        try:
            ...
            # Tolerant on transient sqlite locks
            pass
