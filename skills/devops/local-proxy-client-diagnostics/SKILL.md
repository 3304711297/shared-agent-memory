...
### ⚠️ “延迟”与“网速”是两个独立指标 —— 不能拿握手延迟代表带宽

用户追问：“你只测了延迟，那网速呢”——这是对的。TCP 握手耗时只对应**首连等待**，
不代表下载速度、视频码率或大文件传输能力。

**实测结论（本机，8 个镜像站、钉住各解析器给出的 IP 交错下载）**：
换 DNS 只是换了个“就近节点”，**不能稳定提升带宽**——两目标的胜负方向相反
（腾讯 +25.6% 利好 AliDNS、北大 +15.4% 利好 local）。

⇒ **规则：向用户报告选型收益时，必须区分“哪个指标得到了改善”。**
   首连延迟改善 ≠ 网速改善。若只测了握手，就只能声称握手变快。

### ⚠️ 任何速度对比都要带“同输入对照组”来界定噪声（本条是上面结论成立的前提）

上面那组“IP 不同”的数据本身**不可用于下结论**，因为有反向胜负。
真正定音的是对照组：

| 情形 | 实测差异范围 |
|---|---|
| 两解析器给出**同一 IP**（理论上应无差异） | 1.7% / 4.6% / **16.4%** / **51.2%** |
| 两解析器给出**不同 IP**（待验证的“改善”） | 25.6% / 15.4% |

⇒ **同输入对照组的抖动（最高 51%）盖过了待验证差异（25%）** ⇒ 该差异不可信。
⇒ **规则：做 A/B 性能对比时，必须包含一组“输入相同、结果应相同”的对照，
   先用它量出噪声底线，再判断待测差异是否超出该底线。**
⇒ 没有对照组时，极易把噪声当收益报给用户（本次差点又踩一遍）。
⇒ 配套：每个测点重复 2~3 次取中位、A/B 交错而非分块（抵消瞬时波动）。

### ⚠️ 对比候选值时必须取**客户端内置清单**，不能只测手输的几个

用户纠正：“你对比的不全啊，karing里自带很多可选的服务器”。
第一轮只测了 3 个手输的公共 DNS（`8.8.8.8` / `223.5.5.5` / `119.29.29.29`），
而客户端其实内置了 **30+ 条**预设（`setting_manager.dart` 的常量表，含各家 UDP/DoT/DoH/QUIC 变体）。

⇒ **规则：任何“A 还是 B 更优”的选型任务，先把客户端的候选全集枚举出来再测。**
   开发侧预设常写在源码常量表里（本例 `SettingConfigItemDNS` 列表，行号区间稳定可检索），
   用 `gh search code '<已知值>' --repo <owner>/<repo>` 反查定义点即可拿到全表。
⇒ 不完备的对比会得出误导性结论：漏测的选项里可能就有**更优项**或**直连根本不通的坑**。
   （本次实测发现：内置清单里 4 个 DoT/DoH 选项直连完全不通——把它们填进直连槽就是直接失效。）
⇒ 同时报**成功率**与**速度**：有选项速度漂亮但只有 7/10 成功，不能只按速度排序推荐。

### ⚠️ 手写协议解析器是本类任务最大的误报源（本次连踩两个）

写 DNS/TLS 探测脚本时，**“全部失败”几乎总是解析器 bug，不是真实结论**。本次两个：

| 症状 | 真实原因 | 教训 |
|---|---|---|
| 阿里/Cloudflare DoH 全 0/N | **未处理 CNAME + 名称压缩指针**——响应先给 CNAME（`\xc0\x0c` 指针）再给 A，解析器遇指针崩掉 | 必须实现跳转式域名读取（跟指钕）；有条件优先用成熟库 |
| 所有 DoT 全 0/N | **裸 socket 没调 `connect()` 就先握手** | 握手成功不代表已连接；分步验证用 TCP 先探端口 |

⇒ **处理原则：出现“清一色全失败”时，先用一个已知能通的目标（如 Cloudflare JSON API）
   作为阳性对照，验证脚本本身。** 对照通而目标不通，才是真结论。
⇒ 本次先报了“加密协议全部失败”，若不是逐层复检，就会给用户错误建议。
⇒ 另一个反向坑：DoH 的 `?dns=` 是 **base64url 二进制**（RFC 8484），而 `?name=` 是 Google/Cloudflare
   的 JSON 扩展；拿 `?name=` 测阿里会得到 `no 'dns' query parameter found`——这不是网络问题。
⇒ **写脚本到技能目录时，JSON 转义会把 `\x00` / `\.` 变成字面量双反斜杠**，导致
   `build()` 构造的报文全废、正则匹配不中。`skill_manage(write_file)` 后**必须实跑一次**
   （本次首次运行 20/22 个选项误报失败，就是这个原因）。写完即验，不要只 lint。

### ⚠️ 审计必出**完整清单**，不得只给筛选后的子集

**用户明确要求过“看全部”，而我第一次只报了 176 项里的约 30 项**（挑了自己认为"有优化空间的"），
被追问“你确实全部设置都看完了？为什么好多设置项都没提到”。

⇒ **规则：收到“全面看 / 包括全部”类要求时，先输出完整枚举（含所有无问题的项），
   再在完整列表上标注重点（🔴待改 / 🟡可考虑 / ✅无需动）。不得替用户筛掉"不值得列"的项。**
⇒ 理由：用户无法知道被隐去的是什么，也就无法判断审计完整性；"无需动的项"本身是结论，
   不是噪声——它们正是"我查过了"的证据。
⇒ 实操：用 `execute_code` 递归展开 JSON 的全部叶子键（如 Karing 有 **176 个**叶子键），
   分类打标输出；典型分组：已讲 / 真缺口 / 隐私元数据 / 界面配置。

### 客户端配置备份：本用户不需要

`webdav.*` 全空 + `auto_backup.*` 全关 = 无任何配置备份。
**已确认无需提醒**，用户原话：「备份无所谓，因为机场我随时都能重新获取订阅」。
⇒ 提醒过、用户已表态的事别再重复提（同「用户取舍原则」）。
