...
指标 = 解析出的首个 IP 的 **TCP:443 握手耗时**（对应首连等待，比解析耗时重要）。
...
### ⚠️ 但握手快 ≠ 网速快 —— 换 DNS 能改的是「就近节点」，不是带宽

**用户追问“你只测了延迟，那网速呢”后补测，结论翻盘。**
实测：对 8 个镜像站，钉住各解析器给出的 IP、同一文件交错重复下载（20MB 上限 / 8s）：

**A. IP 不同的目标（DNS 选择真正起作用）：**

| 目标 | local 给的 IP | AliDNS 给的 IP | 谁快 |
|---|---|---|---|
| mirrors.cloud.tencent.com | 111.42.117.168 → 15.41 MB/s | 112.28.174.206 → 20.72 MB/s | AliDNS +25.6% |
| mirrors.pku.edu.cn | 162.105.120.100 → 17.86 MB/s | 111.205.231.79 → 15.12 MB/s | **local +15.4%** |

⇒ **胜负方向相反**：换 DNS 只是换了个“就近节点”，**不能稳定提升带宽**。

**B. 两解析器返回同一 IP 的目标（= 噪声基线，最关键的一组）：**

| 目标 | local | AliDNS | 差异 |
|---|---|---|---|
| mirrors.huaweicloud.com | 16.52 | 16.24 | 1.7% |
| mirror.nju.edu.cn | 14.12 | 14.81 | 4.6% |
| mirrors.bfsu.edu.cn | 19.05 | **22.78** | **16.4%** |
| mirrors.zju.edu.cn | 0.31 | 0.15 | **51.2%** |

⇒ **IP 完全相同、速度仍差 16~51%** ⇒ 单次测速的天然抖动很大；
   没有这组对照，会把 25.6% 的差异误当真实优势（其实在噪声量级内）。
...
| 直连流量 | `local`（可改 `udp://223.5.5.5`） | ⚪ **可改可不改**：首连快16ms，**但与网速无关** |
| 代理流量 | `udp://8.8.8.8`（经节点） | ✅ 最优（明文 UDP 5~6ms vs DoH 267~279ms） |

⇒ **不要拿“优化网速”为由推荐换 DNS**——实测无稳定收益。
   需要告知用户：换的收益仅限**首连握手延迟**（16ms），对下载/视频/大文件无影响。
⇒ 真正影响网速的是：**节点选择**（实测德国比日本生成快 49%）与 CDN 自身的就近调度。
