...
- **Theme Override Dynamic Stylesheet ID**: When injecting CSS overrides into `document.head`, always use a fixed, dedicated element id (e.g. `styleEl.id = 'zcode-desktop-overrides'`) and update `styleEl.textContent` in-place on hot reload rather than appending multiple unbounded `<style>` tags to the DOM.
