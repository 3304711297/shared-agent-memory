...
- **Windows 系统代理与 10054 报错**: 在配置了全局本地代理（如 127.0.0.1:3067）的 Windows 环境下，Python urllib.request.urlopen 默认探测并走系统代理，请求 api.bilibili.com 易偶发 WinError 10054 连接重置。调用时应显式构建空代理处理器 `opener = urllib.request.build_opener(urllib.request.ProxyHandler({}))` 直连 B 站 API。
