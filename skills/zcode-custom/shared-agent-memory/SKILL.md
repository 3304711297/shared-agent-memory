...
- **【同源铁律】`scripts/check_hygiene.py` 在三个位置各有副本，必须逐字节相同**：共享库 main 分支与 hermes 分支（同一工作区、同一文件）、hermes home 仓 hermes 分支。改动任一份后**立即 cp 覆盖其余并 `sha256sum` 比对**，不要各自维护。2026-09-15 实证：两份曾分叉到「路径正则量词 `+`/`*` 不同、占位符白名单 15 vs 26 项」，直接后果是**同一份内容在一个仓扫描通过、在另一个仓 CI 变红**，且 `*` 版会把裸 `C:/Users/`（不含用户名、无泄漏）匹配成空捕获而误报。改检测器时的回归判据（缺一不可）：① 真实违规样本仍被抓（大小写盘符、反向斜杠、真 PAT/AWS/Slack 形态）；② 占位符样本全部放行；③ 两个仓库各跑一次真实扫描。
