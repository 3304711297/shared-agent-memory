...
2. **Hermes 专属事实**（仅本 Agent 运行时需要）→ 更新 `memories/USER.md` 或根 `memories/MEMORY.md`。
   - **【核心常驻记忆与指令纯英文铁律】**：`SOUL.md`、`memories/USER.md` 与根 `memories/MEMORY.md` 的内容必须**严格使用纯英文维护**。严禁任何 Agent 擅自使用中文或中英混杂格式写入，以保证系统级提示词对大模型的锚定强度、最大化指令遵循度并防止 Token/字符膨胀（面向用户的回复正文保持中文即可）。
...
- **描述已被平台截断到约 60 字符上限**（平均 59、最长 60、最短 28）——描述本身已受控，这点常被误估。
- **削减空间极小，不动**：描述上限降到 40 字符仅省 ≈535 token（占系统提示 4.8%），却要重写 90 个受 `check_skill_drift.py` 与看门雷达保护的文件；降到 30 字符描述则失去可读性。技能清单是**固定小额开销**，不随任务膨胀；真正撑爆上下文的是工具输出（实测 `web_extract` 单轮可达 81K 字符）。
...
- **为什么**：技能文件与台账会脱节。实例——`edge-dev-cdp-scraping` 于 09-17 被判定废弃（写入 `notWatched`），但删除动作**漏执行**，文件一直存在；后续 agent 读代码时发现其结论有误便顺手 patch，等于给一个已判死的技能续命、并让两份事实互相矛盾（台账说已删、文件说在用、还被引用）。
- **正确顺序**：① `skill_view` + `.usage.json`（看 `state`/`archived_at`/`last_used_at`）；② 查清单台账确认在役；③ 在役才 patch；已判废弃则**报告用户**并按其决定走「补齐删除 / 复活 / 仅修错 」三选一，不得自决。
- **同类隐患**：台账记「已删除/已废弃」≠ 文件真的没了。凡看到此类记录，顺手核一次磁盘与 git 跟踪状态；不一致就一起收口。
