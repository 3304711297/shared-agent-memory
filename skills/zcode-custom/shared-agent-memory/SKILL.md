...
- **共享库物理真源**：`C:\Users\<username>\.zcode\cli\memories\projects\default-135ef1b9f66d8a7e\memory\`
- **远程仓库**：`https://github.com/3304711297/shared-agent-memory.git`，**共享内容在 `main` 分支（默认分支）**
- **本 Agent 视角（等价路径）**：`C:\Users\<username>\AppData\Local\hermes\memories\topics\` = 上述真源的 junction
...
3. **【铁律】修改或新增记忆后，当轮结束前自动静默推送**（无需等待用户提醒；直连失败回退 `-c http.proxy=http://127.0.0.1:3067`）：
   ...
   ```bash
   git -C "C:/Users/<username>/.zcode/cli/memories" add -A && git -C "C:/Users/<username>/.zcode/cli/memories" commit -m "memory: <简述>" && git -C "C:/Users/<username>/.zcode/cli/memories" push origin main
   ```
   - Hermes 专属内容（自家 home 仓库）：
   ```bash
   git -C "C:/Users/<username>/AppData/Local/hermes" add -A && git -C "C:/Users/<username>/AppData/Local/hermes" commit -m "docs(memory): <简述>" && git -C "C:/Users/<username>/AppData/Local/hermes" push origin hermes
   ```
...
- **服务拓扑**：
  - **OpenViking 核心服务**：`http://127.0.0.1:1933`（独立虚拟环境 `C:\Users\<username>\.openviking\venv`，无黑框后台运行）
...
- **双驱动自动同步**：
  1. **即时驱动（Git Hook）**：在 `C:\Users\<username>\.zcode\cli\memories\.git\hooks\post-commit` 与 `post-merge` 挂接 `sync_shared_memory_openviking.py`，ZCode 或本地有 commit 产生时秒级增量触发 OpenViking 重新扫描；
...
- **后台服务守护与运维**：
  - 查看状态：`python C:/Users/<username>/AppData/Local/hermes/scripts/openviking_service.py status`
  - 启停服务：`python C:/Users/<username>/AppData/Local/hermes/scripts/openviking_service.py [start|stop|restart]`
