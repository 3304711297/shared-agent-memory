...
- **PowerShell 7 Pipeline Stop on Native Executables (`$LASTEXITCODE` cleared to `$null`)**:
  In PowerShell 7 (`pwsh`), piping native executable output directly into `Select-Object -First 1` (e.g. `(& git rev-parse HEAD 2>&1 | Select-Object -First 1)`) terminates the pipeline upstream early once the first item is emitted. This leaves native command exit status unrecorded or cleared, resulting in `$LASTEXITCODE` becoming `$null`. In PowerShell logic, `$null -ne 0` evaluates to `$true`, which will falsely trigger non-zero exit handlers (e.g. reporting valid git checkouts as non-git repositories).
  *Rule*: Always capture native command output into a variable first (`$out = (& git rev-parse HEAD 2>&1)`), verify `$LASTEXITCODE`, then extract the desired line (`$sha = ($out | Select-Object -First 1)`).
