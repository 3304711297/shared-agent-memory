...
Recurring user question: "Can app X (Karakeep, OpenWebUI, LibreChat, a memory
plugin, LangChain pipeline, n8n flow, etc.) use my Nous Portal subscription
without me copy-pasting an API key — ideally via the Portal login I already
have?"
...
This is the question to answer FIRST. Any plugin under `plugins/` (memory
providers, tool adapters) trips agents up here.

| Surface | What it actually is | Auth path |
|---|---|---|
| **A Hermes plugin** (e.g. `plugins/memory/<name>/`) | Code that runs **inside the Hermes process**. Its LLM calls go through Hermes's already-configured provider. | Already uses Portal if user's Hermes is configured for Portal. Nothing extra needed. Any `*_API_KEY` it reads for its *own* backing service is not LLM auth. |
| **That service's standalone server** (separate container) | A separate service. If it ever calls an LLM on its own, that's a separate HTTP client. | Same as any external app — Layer 2/3 below. |
...
via Hermes's provider config. The plugin's own server auth (e.g. an
`*_API_KEY` for talking to its backing service's REST API) is unrelated to
Portal.
