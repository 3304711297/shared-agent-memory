...
- **网络/代理节点切换导致「连接已中断」时的刷新自愈（2026-09-22 拍板）**：当网络变动或代理节点切换时，SSE 长连接中断，页面会呈现「连接已中断。正在等待完整回复」或停止生成。此时**标准应对动作是直接刷新当前会话标签页**（通过 `evaluate` 调用 `location.reload()` 或重新直达当前 `/c/<id>` URL）。刷新后 ChatGPT 会直接拉取后端已完整落库的最终回答，切勿以为回答丢失或在断开状态下重复发送。
- **超长审查输出读取避坑（2026-09-23 实战）**：当 ChatGPT 输出长达万字（如 1.5~2 万字符）的高密度深度审查报告时，直接通过 `bsk evaluate` 将完整字符串单次作为 stdout 回传可能因 Windows 命令行 buffer 或特殊字符转义导致解析异常或静默为空；**标准做法是通过 DOM 分块切片（如每次取 2000 字符 `innerText.slice(i, i+2000)`）拉取并安全拼装落盘到本地临时文件，再通过 `read_file` 研读**。
- **思考模式激活严格验证（2026-09-23 实战）**：点击「思考」按钮（`button[aria-label*="思考"]`）后，严禁凭意图假设已生效，必须立即通过 `evaluate` 检查并断言其 `aria-pressed === 'true'`，确认推理模式真正激活后再提交审查提要。
