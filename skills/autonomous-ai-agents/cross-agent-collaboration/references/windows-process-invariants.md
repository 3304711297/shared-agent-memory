...
2. **Tier 2: System-Level Targeted Whitelist Reaper & Agent Guard (Exit Failsafe)**:
   ...
   - Implementation: canonical safe reaper at `%LOCALAPPDATA%/hermes/scripts/cleanup_agent_orphans.py`, orchestrated by background daemon `%LOCALAPPDATA%/hermes/scripts/agent_guard.py` (2.5s debounce after all Agent GUIs close). Both run on the Hermes venv python (`%LOCALAPPDATA%/hermes/hermes-agent/venv`) — **not** on any memory-service venv (OpenViking retired 2026-09-21).
