...
### Model selection & Small model recommendation

| Size | Parameters | VRAM | Relative Speed | Recommended Usage |
|:---|:---|:---|:---|:---|
| `tiny` | 39 M | ~1 GB | ~32x | Quick drafts, low resources |
| `base` | 74 M | ~1 GB | ~16x | Good balance for English |
| `small` | 244 M | ~2 GB | ~6x | **🌟 最推荐主力模型 (Default/Preferred)**：中文及多语种语音识别准确率与推理速度的黄金平衡点 |
| `medium` | 769 M | ~5 GB | ~2x | High accuracy, heavier |
| `large-v3` | 1550 M | ~10 GB | 1x | Maximum accuracy |

> **用户环境实践原则**：在本地转录 B 站视频、硬件科普或日常音频时，**优先默认推荐使用 `small` 模型**（显存占用仅约 2GB，中文断句与生僻词识别远优于 base，且速度远快于 medium/large）。
