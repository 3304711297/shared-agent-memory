...
- **EasyCLIProxyAPI 多账号调度优先级与会话粘性规约**：
  - **优先级数值规则**：在 `auth/*.json` 中，`"priority"` 为整数，**数值越大优先级越高**（例如 Priority 10 高于 Priority 9）。在 `routing.strategy: fill-first`（优先填充）模式下，流量无条件优先打满高优先级账号配额。
  - **轮询策略的优先级硬分组陷阱（Round Robin Priority Bucket）**：当 `routing.strategy` 改为 `round-robin`（轮询）时，EasyCLIProxyAPI 内核（`getSelectorAvailableAuthsWithPriorityMode` -> `highestPriorityAuths`）**只在当前最高优先级的分组内部轮询**，不会跨优先级分发！若两个账号优先级不同（如 10 与 9），轮询池内仅有优先级 10 的单一账号，轮询行为退化为单号单打。**若要在多账号间实现真正的负载均衡轮询，必须将参与轮询的所有凭据优先级数值设为完全一致**（如全部设为 10）。
  - **会话粘性高于优先级**：开启 `routing.session-affinity: true`（默认 TTL 1h）后，会话首次建立的凭据绑定**压倒优先级规则**（`established binding outranks credential priority`）。只要旧会话未超时，即使高优先级账号恢复或低优先级账号告警，当前会话依然固定复用已绑定的凭据。在轮询+会话粘性下，新开会话（`session-affinity: LCP cache miss, new binding`）按账号轮流绑定，单会话内部多轮追问（`session-affinity: LCP cache hit`）牢牢绑定单号以最大化 Prompt/KV Cache 命中率。
  - **调用归属唯一真源**：排查当前实际调用哪个账号时，严禁依据客户端显示名称或状态栏弹窗猜测。必须核验网关物理数据库 `D:/EasyCLIProxyAPI-v0.2.71-Windows-amd64/usage-records/usage.db`（`SELECT source, count(*), max(timestamp) FROM usage_events GROUP BY source`）或实时日志 `auth/logs/main.log`（`auth=antigravity-...json`）作为唯一客观证据。
...
- **Direct Quota Architecture**:
  ...
  - **多凭据多账号池动态跟随架构（已打通闭环）**：
    - 后端 `plugin_api.py` 内置 `_get_active_email()`：通过只读连接网关物理数据库 `usage-records/usage.db`（查询 `usage_events` 表最新 `source`）及 `auth/logs/main.log`，动态识别当前真正承接调用的活跃凭据，不再按字母序盲读首个文件。
    - 并发批量拉取：利用 `ThreadPoolExecutor` 并发直连 Google 官方 `daily-cloudcode-pa` 接口扫描所有有效凭据，返回包含 `isActive` 标识的 `accounts` 全量列表。
    - 前端 UI 呈现：状态栏 Popover 与全景看板自动渲染「凭据池 (N 账号)」卡片，展示各账号的 5h/周配额，并高亮标记「● 当前活跃路由」；订阅方案标签自适应识别当前活跃账号是否为 Pro。
  - **排查真实调用账号的客观依据**：无论前端如何展示，底层验证真实调用账号时，必须以网关物理数据库 `D:/EasyCLIProxyAPI-v0.2.71-Windows-amd64/usage-records/usage.db`（`SELECT source, count(*), max(timestamp) FROM usage_events GROUP BY source`）或实时日志 `auth/logs/main.log`（`auth=antigravity-...json`）作为唯一客观证据。
...
- **Extension Clear Pitfall & Chromium Garbage Collection Mechanism**:
  - **Cold Launch Hazard**: If an automation tool or MCP (e.g. `chrome-devtools-mcp` configured with `--user-data-dir`) is triggered while the user's primary Edge/Chrome instance is closed, Chromium cold-launches the profile in headless/automation mode without activating regular extensions.
  - **Preference Table Overwrite**: Upon process exit, Chromium flushes its in-memory state back to disk, overwriting `Secure Preferences` (`extensions.settings`) with an empty extension registry.
  - **Physical Extension Directory Eviction**: On the subsequent normal browser startup, Chromium's internal `ExtensionGarbageCollector` (logged in Windows Event Log as `Garbage collection for extensions on file thread is complete`) scans `Default/Extensions`. Detecting directories that lack active registrations in `Secure Preferences`, it classifies them as orphaned and **physically deletes the extension folders from disk**.
- **Data-Code Decoupling & Instant Recovery Protocol**:
  - **Physical Data Remains 100% Intact**: Chromium strictly decouples extension program binaries (`Default/Extensions/<id>`) from user persistent databases (`Default/Local Extension Settings/<id>` and `IndexedDB/chrome-extension_<id>_0.indexeddb.leveldb`). The garbage collector only wipes binaries; user data, scripts, and local settings are completely untouched.
  - **Store Extensions (ScriptCat, KISS Translator, Immersive Translate)**: Re-acquire the extension from Microsoft Edge Add-ons (or Chrome Web Store). Because extension IDs are globally constant, the freshly downloaded extension immediately rebinds to the existing `Local Extension Settings` LevelDB upon installation. All userscripts (e.g. Hugging Face, SteamDB, OpenRouter translations) and user configurations are 100% restored in place without requiring manual re-importing.
  - **Unpacked Extensions (e.g. `D:\extensions\` with fixed `key`)**: Re-load via `edge://extensions` ("Load unpacked"). As long as the `manifest.json` contains a pinned `key` (matching the original extension ID), local settings reattach immediately.
- **Hardening Rules & Configuration Invariants**:
  - **The `--user-data-dir` Convenience Trap**: Users often pass `--user-data-dir` to `chrome-devtools-mcp` under the misconception that it will seamlessly reuse existing extensions without requiring manual browser launches. In reality, any headless or automated cold-launch against the live profile exits in an uninitialized extension state, flushing empty preferences and triggering Chromium's `ExtensionGarbageCollector` to physically erase installed extension binaries.
  - **No `--user-data-dir` on Primary Profile in MCP**: In `config.yaml` under `mcp_servers.chrome-devtools`, **strictly omit `--user-data-dir`**. Only specify `--autoConnect` (plus extension protection args like `--ignore-default-chrome-arg=--disable-extensions`). If Edge is closed, the MCP must fail fast with a connection error and wait for the user to open the browser, rather than autonomously launching an automation instance that stamps on the live profile.
  - **Disable Native Browser Automation in Desktop GUI**: When dedicated search/extraction engines (e.g. Exa) are active, explicitly toggle OFF the **\"Browser Automation\"** switch in the Hermes Desktop settings (`platform_toolsets.cli` removes `browser`). This prunes all 13 `browser_*` primitive tools from the agent's toolset, eliminating any accidental browser invocation by background subagents.
  - Set `browser.use_real_profile: false` in `config.yaml` to guarantee built-in browser automation runs strictly in an isolated disposable sandbox.
