  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      const res = await host.request('system.read_file', {
        path: 'C:/Users/<username>/AppData/Local/ZCodeAntigravity/quota-cache.json'
      }).catch(() => null)

      const metricsRes = await host.request('system.read_file', {
        path: 'C:/Users/<username>/AppData/Local/ZCodeAntigravity/usage-metrics.json'
      }).catch(() => null)
      ...
      if (q5h == null) {
        const directRes = await host.request('system.read_file', {
          path: 'C:/Users/<username>/AppData/Local/hermes/desktop-plugins/token-stats/direct-quota.json'
        }).catch(() => null)
